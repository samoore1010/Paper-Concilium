...
/**
 * Static persona prompts for persona packs (Legal Bench, Business Tank) not yet
 * integrated into the personality engine. These serve as a fallback.
 */
...
/**
 * Get the composed prompt for a persona. Tries the personality engine first
 * (multi-layered composition), falls back to static PERSONA_PROMPTS for persona
 * packs not yet in the engine. Defaults to "maria-chen" via the engine.
 */
export function getPersonaPrompt(personaId: string, sessionType = "business-pitch"): PersonaPrompt {
  ...
  if (PERSONA_PROMPTS[personaId]) return PERSONA_PROMPTS[personaId];
  return composePersonaPrompt("maria-chen", sessionType) as PersonaPrompt;
}
